prepare_HPO <- function(save_path=tempfile(pattern = "HPO_seurat",
                                           fileext = ".rds"),
                        as_seurat=TRUE){
    HPO <- data.table::fread(
        file.path("https://ci.monarchinitiative.org/view/hpo/job",
    ...
    colnames(HPO) <- gsub("-|[ ]","_",
                          stringr::str_split(
                              ...
                              "<tab>")[[1]])
    HPO <- HPO %>% dplyr::mutate(HPOid=HPO_id,
                                 HPO_id=gsub("[:]",".",HPO_id)) 
    #### Prepare metadata ####
    meta <- HPO %>% dplyr::group_by(HPO_id,HPOid, HPO_label) %>%
    ...
    #### Assign groups ####
    meta_groups <- get_ontology_groups(ontology = hpo, 
    ...
    if(as_seurat){
        obj <- scNLP::seurat_pipeline(counts = mat,
                                      meta.data = meta, 
