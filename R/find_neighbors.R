...
find_neighbors <- function(obj,
  ...
  if(!is.null(var1_search)){
    messager("+ Filtering results by var1_search:",
             var1_search,v=verbose)
    targets1 <- grep(pattern = var1_search, x = sample_names,
    ...
    } else {stop("0 entries in label_col match the ",
  ...
  if(!is.null(group_col) && (group_col %in% colnames(metadata))){
    ...
    targets2 <- metadata[grepl(var2_group, metadata[[group_col]], 
                               ignore.case = TRUE),
                                 "sample_names"]
    ...
    } else {stop("0 entries in group_col match the ",
  ...
  top_candidates <-  fgraph %>%
    ...
    dplyr::slice_max(order_by = similarity,
                     n = max_neighbors) %>% 
