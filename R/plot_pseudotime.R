...
#' @param obj A \pkg{Seurat} object of trait data 
...
#' @param max_disease_ids Maximum number of \code{disease_ids} to include.
...
plot_pseudotime <- function(obj,
    ...
    disease_ids <- map_id_sep(unique(disease_ids))|>na.omit()|>as.character()
    if(!is.null(max_disease_ids)){
        disease_ids <- utils::head(disease_ids, max_disease_ids)
    }
    #### Run pseudotime: merged ####
    if(isTRUE(merge_trajectories)){
        out <- run_pseudotime(obj = obj, 
        ...
        messager("Running pseudotime analysis for:", 
                 length(disease_ids),"disease_ids.") 
        out <- list()
        out[["subplots"]] <- lapply(stats::setNames(disease_ids,
          ...
          run_pseudotime(obj = obj, 
                         disease_ids = d,
                         p2d = p2d, 
                         id_col = id_col,
                         root_cells = if(is.null(root_cells)) d else root_cells,
                         map_root_cells=map_root_cells,
                         learn_graph_control = learn_graph_control,
                         use_partition=use_partition,
                         return_cds=return_cds,
