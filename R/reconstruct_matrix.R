...
reconstruct_matrix <- function(u_name="contribution_phe",
    ...
    if(gene_level){ 
        ...
        label_var_gene <- annotate_vector(vec =  DEGAS["label_var"],
                                          annot = annot) %>% unname() 
    }   
    ...
    u <- u %>% `rownames<-`(label_phe_code) %>% `colnames<-`(label_dim)
    v <- DEGAS[v_name] %>% `rownames<-`(label_var_gene) %>% `colnames<-`(label_dim)
