...
map_snps2genes <- function(sumstats_file,  
    ...
    if(methods::is(sumstats_file,"character")){
        ...
        sumstats <- data.table::fread(sumstats_file, 
                                      nThread = nCores)    
    } else {sumstats <- sumstats_file}
    #### Select method and map SNPs ####
    #### txdb ####
    if(method=="txdb"){
        #### Map ####
        gene_hits <- map_snps2genes_txdb(sumstats = sumstats,
                                         verbose = verbose) 
        #### Adjust ZSTAT ####
        gene_hits <- adjust_zstat(dat = gene_hits,
                                  model = model, 
                                  log_vars = log_vars,
                                  drop_MHC = drop_MHC,
                                  formula = formula,
    ...
    if(method=="abc"){
        #### Map ####
        gene_hits <- map_snps2genes_abc(sumstats = sumstats,
                                        abc = abc,
                                        dataset = dataset,
                                        nCores = nCores,
                                        verbose = verbose)
    ...
    #### Save ####
    out_path <- save_snps2genes(gene_hits = gene_hits,
                                save_dir = save_dir,
    ...
    report_time(start)
    #### Return ####
    if(return_path) return(out_path) else return(gene_hits)
}
