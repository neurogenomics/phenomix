filter_matrix <- function(X,
    ...
    if(!is.null(colsum_min)){
        X <- X[,unname(Matrix::colSums(X, na.rm = TRUE)>=colsum_min),drop=FALSE]  
    } 
    ## Filter out rows with all zeros
    if(!is.null(rowsum_min)){
        X <- X[as.numeric(Matrix::rowSums(X, na.rm = TRUE))>=rowsum_min,,drop=FALSE] 
    }  
    ## Filter columns by number of non-zero entries
    if(!is.null(rown_min)){
        X_nfeat <- apply(X,2, function(x) sum(x>0, na.rm = TRUE))  
        X <- X[,unname(X_nfeat)>=rown_min,drop=FALSE]  
    }
    ## Filter rows by number of non-zero entries
    if(!is.null(coln_min)){
        X_nsamp <- apply(X,1, function(x) sum(x>0, na.rm = TRUE))  
